...
let moveLeft = false;
let moveRight = false;
const isTouchDevice = matchMedia("(pointer: coarse)").matches || "ontouchstart" in window || Math.min(innerWidth, innerHeight) <= 900;
...
function setupMobileControls() {
  ...
  const weaponButton = document.getElementById("mobileWeapon");
  const pauseButton = document.getElementById("mobilePause");
  ...
  moveZone.addEventListener("pointerdown", (event) => {
    if (gameState !== "playing" || movePointer !== null) return;
  ...
  moveZone.addEventListener("pointermove", (event) => {
    ...
    moveLeft = dx < -dead; moveRight = dx > dead;
    moveForward = dy < -dead; moveBackward = dy > dead;
  });
  moveZone.addEventListener("pointerup", stopMove);
  moveZone.addEventListener("pointercancel", stopMove);

  lookZone.addEventListener("pointerdown", (event) => {
    if (gameState !== "playing" || lookPointer !== null) return;
  ...
  lookZone.addEventListener("pointermove", (event) => {
    ...
    camera.rotation.x = Math.max(-Math.PI / 2, Math.min(Math.PI / 2, camera.rotation.x - dy * 0.004));
  });
  ...
  const stopFire = () => { mouseHeld = false; isAutoFiring = false; };
  ...
  weaponButton.addEventListener("pointerdown", (event) => {
    event.preventDefault(); event.stopPropagation();
    if (gameState === "playing") switchWeapon();
  });
  pauseButton.addEventListener("pointerdown", (event) => {
    event.preventDefault(); event.stopPropagation(); pauseGame();
  });
...
function pauseGame() {
  ...
  if (isTouchDevice) document.getElementById("mobileControls").classList.remove("active");
...
function resumeGame() {
  ...
  hideAllMenus();
  document.getElementById("gameUI").style.display = "block";
  if (isTouchDevice) document.getElementById("mobileControls").classList.add("active");
